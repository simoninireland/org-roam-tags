...
** Links, backlinks, and content tags
   ...
   ~org-roam~ mode provides a database layer on top of Emacs ~org~
...
** In use
   ...
   ~org-roam-tags-tag-regexp~ which defaults to notes with
   single-word titles consisting only of lower-case letters, digits,
   and dashes (~(rx (seq bol (one-or-more (in lower digit "-")) eol))~).
